...
struct ValidationStateMachine
{
   ...
   static constexpr auto s_receive_wait = boost::sml::state<class StateWaitReceiveDone>;
   ...
   auto operator()() const
   {
      ...
      return make_transition_table(
          ...
          *s_disable_i2c   + e_tick                                    / disable_i2c                            = s_wakeup,
          ...
          s_read_id        + e_tick                                    / (reset_timer, read_id)                 = s_receive_wait,

          s_receive_wait   + e_receive_done                            / store_id                               = s_verify_id,
          s_receive_wait   + e_tick          [!receive_wait_timeout]   / tick_timer,               
          s_receive_wait   + e_tick          [receive_wait_timeout]    / (set_bus_error, mark_validation_fail)  = X,               
