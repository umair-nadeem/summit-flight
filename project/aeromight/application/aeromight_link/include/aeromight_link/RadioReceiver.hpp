...
template <typename QueueReceiver, typename QueueSender, typename CrsfRcChannelsParser, interfaces::IClockSource ClockSource, typename Logger>
class RadioReceiver
{
public:
   ...
   void execute()
   {
      ...
      if (queue_item.has_value())
      {
         ...
         // m_logger.printf("size      %u", bytes.size());
         if (bytes.size() == 26)
         {
            if (CrsfRcChannelsParser::crsf_parse(bytes, m_rc_channels))
            {
               m_counter++;
               if (m_counter % 10 == 0)
               {
...
private:
   ...
   Logger&                                                        m_logger;
   std::array<uint16_t, CrsfRcChannelsParser::crsf_channel_count> m_rc_channels{};
